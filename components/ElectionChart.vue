<template>
    <div class="w-full space-y-6">
      ...
      <div class="space-y-4">
        ...
        <div v-for="(candidates, position) in props.electionData" :key="position" class="p-4 bg-gray-100 space-y-3 rounded shadow">
          <h3 class="text-md font-semibold">{{ formatPosition(position) }}</h3>
         <div class="space-y-5">
  ...
  const generateChartData = () => {
    ...
    Object.keys(props.electionData).forEach((position) => {
      const totalVotes = props.electionData[position].reduce((sum, candidate) => {
